...
//we are done, reset
    ...
    public static float[] changeMVPMatrix(float[] mvpMatrix, float viewWidth, float viewHeight, float textureWidth, float textureHeight) {
        //        float scale = viewWidth * textureHeight / viewHeight / textureWidth;
        float scaleW = textureWidth / viewWidth;
        float scaleH = textureHeight / viewHeight;
        float[] mvp = new float[16];
        float[] tmp = new float[16];
        Matrix.setIdentityM(tmp, 0);
        Matrix.scaleM(tmp, 0, scaleW, scaleH, 1F);
        Matrix.multiplyMM(mvp, 0, tmp, 0, mvpMatrix, 0);
        return mvp;
    }

    public static float[] getRotataionMatrix(float[] texMatrix, int rotation) {
        float[] tex = new float[16];
        float[] tmp = new float[16];
        Matrix.setIdentityM(tmp, 0);
        Matrix.setIdentityM(tmp, 0);
        Matrix.translateM(tmp, 0, 0.5f, 0.5f, 0f);
        Matrix.rotateM(tmp, 0, rotation, 0f, 0f, -1f);
        Matrix.rotateM(tmp, 0, rotation, 0f, -1f, 0f);
        Matrix.translateM(tmp, 0, -0.5f, -0.5f, 0f);
        Matrix.multiplyMM(tex, 0, tmp, 0, texMatrix, 0);
        return tex;
    }
